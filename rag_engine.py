...
elk_index_name = "rag_engine"#"tutorial76"
...
def embeddings_on_elasticsearch(texts):
    # pinecone.init(
    ...
    #     texts, embeddings, index_name=st.session_state.pinecone_index)
    ...
    vector_db_store = ElasticsearchStore(
        ...
        index_name=elk_index_name,  # Nombre del índice existente
        es_cloud_id=ELASTIC_CLOUD_ID,  # ID de la nube de Elasticsearch
        es_api_key=ELASTIC_API_KEY # Clave API para autenticación
    )
...
def input_fields():
    ...
    #             "Pinecone index name")
    #
    st.session_state.pinecone_db = st.toggle('Use Elasticsearch Vector DB')
    #
    ...
    try:
        for source_doc in st.session_state.source_docs:
            ...
            if not st.session_state.pinecone_db:
                ...
                st.session_state.retriever = embeddings_on_elasticsearch(texts)
